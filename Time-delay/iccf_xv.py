...
data1 = np.loadtxt("data_use/V_58380_0620.txt")
data2 = np.loadtxt("data_use/corona_58380_1sigma.txt")
...
t, r, rmax, tau_cent, tau_peak = piccf_mc.piccf(jdl, fl, jdc, fc, 1001, -20.0, 30.0)


ax1.plot(t, r,color='red')
ax1.set_xlabel("lag (days)",fontsize=28)
ax1.set_ylabel("ICCF",fontsize=28,color='red')

#ax = fig.add_subplot(212)

ax2 = ax1.twinx()
tau_cent_mc, tau_peak_mc = piccf_mc.piccf_mc(jdc, fc, efc, jdl, fl, efl, 1001,-20, 30.0, 5000)
...
tau_peak_mc[tau_peak_mc==0]=np.nan
tau_cent_mc[tau_cent_mc==0]=np.nan

np.seterr(divide='ignore')
bins = np.arange(0,30,1.70)
print(tau_peak_mc)
print(bins)
#ax2.hist(tau_cent_mc, density=True, bins=bins, label='centroid',histtype='step')
#ax2.hist(tau_peak_mc, density=True, bins=bins, stacked = True,alpha=0.5, label='peak',histtype = 'step')
...
ax2.set_ylabel("density", fontsize=28,color='blue')
#ax.set_xlabel("lag")
ax2.legend(frameon=0,loc='upper left',fontsize=28)


#ax2.set_xlim(-10,10)
#ax2.set_ylabel("density", fontsize=15,color='black')
ax2.set_xlabel("lag (days)", fontsize=28)
...
ax1.text(27,0.75,"B",fontsize=30,fontweight='bold')
...
#plt.show()
plt.title("$\it{V}$-band  vs.  X-ray",fontsize=28)
plt.savefig("xv.png",dpi=800,bbox_inches='tight')
plt.show()
#print(tau_cent_mc)
